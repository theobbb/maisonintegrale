    ...
    <>
      <Box
        ...
        sx={{
          ...
          width: 72,
          height: 72,
          transition: "transform cubic-bezier(0.43, 0.13, 0.23, 0.96) .8s",
          transform: "translateX(-300px)",
          ...(drawerOpen
            ? { opacity: 1, transform: "translateX(0)", transitionDelay: ".8s" }
      ...
      <Box
        sx={{
          ...
          width: { lg: 46, xl: 80 },
          zIndex: 1000,
          transition: "cubic-bezier(0.43, 0.13, 0.23, 0.96) .8s",
          transform: "translateX(-100px)",
          ...(!matchDownLG && !hideLogo
            ? { opacity: 1, transform: "translateX(0)", transitionDelay: ".2s" }
      ...
      >
        ...
        >
          {matchDownLG && <MenuButton {...{ drawerOpen, setDrawerOpen }} />}
          <Box
            sx={{
              paddingLeft: {
                xl: hideLogo ? 2.8 : 8.4,
        ...
        >
          {linkPaths[router.locale]
            .slice(matchDownLG ? 0 : 1)
            .map((path, index) => (
              <NavLink
                index={index + 1}
                href={path.name}
...
function Logo({ style }) {
  ...
  const maison = {
    stroke:
      mode == "light" ? "rgba(0, 0, 0, 0.8)" : "rgba(255, 255, 255, 0.72)",
  };
  const integrale = {
    fill: mode == "light" ? "rgba(73, 138, 46, 0.9)" : "rgba(73, 138, 46, 1)",
  };
  return (
    <svg
      style={{
      ...
      viewBox="0 0 214.85 313.13"
    >
      <title>logo</title>
      <g style={{ fill: "none" }}>
        <polygon
          style={{ ...maison, strokeMiterlimit: 10, strokeWidth: "16px" }}
          points="8.36 119.44 8.36 238.15 189.93 238.15 189.93 119.44 98.97 48.66 8.36 119.44"
        />
      </g>
      <g>
        <path
          style={integrale}
          d="M90.42,283.92C76.49,306.76,60.85,303.84,60,305.39A22.9,22.9,0,0,1,40,317C20,316.59,7.22,302.68,7.21,301.07c0-2.28,11.57-12.56,33.43-12.82,12.67-.15,17.67,9.48,19,9.32,6.11-.76,10.14-5.31,13.74-10,11.55-15,18.44-78.79,28.89-129.24S122.55,63.74,138.86,37c13.93-22.84,29.57-19.92,30.43-21.47A22.93,22.93,0,0,1,189.23,3.89c20.07.42,32.83,14.33,32.84,16,0,2.28-11.57,12.56-33.43,12.82-12.67.15-17.67-9.49-19-9.32-6.12.76-10.14,5.31-13.75,10-11.54,15-21.4,87.21-29.17,127C121.55,186.58,106.73,257.17,90.42,283.92Z"
          transform="translate(-7.21 -3.89)"
        />
      </g>
      <g>
