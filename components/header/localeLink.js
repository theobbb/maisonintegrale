  ...
  useEffect(() => {
    function getOtherQuery() {
      ...
      const otherQuery = queries.find(
        (query) => query.slug[router.locale].current == router.query.slug
      )?.slug[props.locale]?.current;
    ...
    if (router.asPath == "/")
      return setOtherPath(`/${props.locale == "fr" ? "" : "en"}`);

    const pathIndex = linkPaths[router.locale].indexOf(
      linkPaths[router.locale].find(
        (path) => format(path.name) == format(router.asPath)
      )
    );
    if (pathIndex != -1)
      return setOtherPath(linkPaths[props.locale][pathIndex].name);
